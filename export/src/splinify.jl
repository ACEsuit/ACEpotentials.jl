...
# `ETModels._convert_agnesi` fills.  Both are used by `extract_hermite_spline_data` below AND
# by every per-pair table writer in write_radial.jl -- they live HERE rather than there
# because the diagnostic scripts under export/scripts/ include splinify.jl + codegen.jl
# without write_radial.jl, and export_ace_model.jl includes splinify.jl first.
# ----------------------------------------------------------------------------------------
