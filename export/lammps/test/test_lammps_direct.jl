#!/usr/bin/env julia
"""
Direct LAMMPS test for ACE/Minimal plugin.

This test:
1. Exports a simple ACE model using minimal export
2. Creates LAMMPS input files
3. Runs LAMMPS with the ACE/Minimal pair style
4. Validates that LAMMPS runs successfully
"""

using ACEpotentials
using ACEpotentials.ETModels
using ACEpotentials.ETModels: splinify, ETOneBody, StackedCalculator, one_body
using StaticArrays
using Random
using Lux
using LuxCore
using AtomsBase
using Unitful
using AtomsBase: ChemicalSpecies

const M = ACEpotentials.Models
const ETM = ACEpotentials.ETModels

# Load export function
export_dir = dirname(dirname(@__DIR__))
include(joinpath(export_dir, "src", "export_ace_model_minimal.jl"))

println("\n" * "="^80)
println("LAMMPS ACE/Minimal Direct Test")
println("="^80)

# Check environment
lammps_exe = get(ENV, "LAMMPS_EXE", nothing)
if lammps_exe === nothing || !isfile(lammps_exe)
    @error "LAMMPS executable not found. Set LAMMPS_EXE environment variable."
    exit(1)
end

c_interface_path = get(ENV, "ACE_C_INTERFACE_PATH", nothing)
if c_interface_path === nothing || !isfile(c_interface_path)
    @error "ACE_C_INTERFACE_PATH not set or file not found."
    exit(1)
end

println("\n[1] Creating test model...")
elements = (:Si,)
rcut = 5.5
rng = Random.MersenneTwister(1234)

ace_model = M.ace_model(;
    elements = elements,
    order = 2,
    Ytype = :solid,
    level = M.TotalDegree(),
    max_level = 8,
    maxl = 2,
    pair_maxn = 8,
    rin0cuts = (x -> (rin = x.rin, r0 = x.r0, rcut = rcut)).(M._default_rin0cuts(elements)),
    init_WB = :glorot_normal,
    init_Wpair = :glorot_normal
)

ps, st = Lux.setup(rng, ace_model)

# Convert to ETACE
println("[2] Converting to ETACE...")
et_model = ETM.convert2et(ace_model)
et_ps, et_st = LuxCore.setup(rng, et_model)

# Copy parameters
for iz in 1:1, jz in 1:1
    et_ps.rembed.post.W[:, :, (iz-1)*1 + jz] .= ps.rbasis.Wnlq[:, :, iz, jz]
end
for iz in 1:1
    et_ps.readout.W[1, :, iz] .= ps.WB[:, iz]
end

# Splinify
println("[3] Splinifying model...")
et_model_splined = splinify(et_model, et_ps, et_st; Nspl=50)
et_ps_splined, et_st_splined = LuxCore.setup(rng, et_model_splined)

for iz in 1:1
    et_ps_splined.readout.W[1, :, iz] .= et_ps.readout.W[1, :, iz]
end

# Create ETACE calculator (WITHOUT E0 to avoid the forces issue)
etace_calc = ETM.ETACEPotential(et_model_splined, et_ps_splined, et_st_splined, rcut)

# Export model
println("\n[4] Exporting model...")
model_dir = joinpath(@__DIR__, "build", "lammps_direct_test")
export_ace_model_minimal(etace_calc, model_dir; model_name="ace_si")

# Create test structure (LAMMPS data file)
println("\n[5] Creating LAMMPS data file...")
data_file = joinpath(@__DIR__, "build", "test_structure_direct.data")
mkpath(dirname(data_file))

open(data_file, "w") do io
    println(io, "LAMMPS data file for ACE/Minimal direct test")
    println(io, "")
    println(io, "2 atoms")
    println(io, "1 atom types")
    println(io, "")
    println(io, "0.0 10.0 xlo xhi")
    println(io, "0.0 10.0 ylo yhi")
    println(io, "0.0 10.0 zlo zhi")
    println(io, "")
    println(io, "Masses")
    println(io, "")
    println(io, "1 28.0855")
    println(io, "")
    println(io, "Atoms")
    println(io, "")
    println(io, "1 1 0.0 0.0 0.0")
    println(io, "2 1 3.0 0.0 0.0")
end

# Create LAMMPS input script
println("\n[6] Creating LAMMPS input script...")
input_file = joinpath(@__DIR__, "build", "test_direct.lammps")
plugin_path = abspath(joinpath(dirname(@__DIR__), "plugin", "lib", "libaceminimal.so"))

open(input_file, "w") do io
    println(io, "# LAMMPS input for ACE/Minimal direct test")
    println(io, "units metal")
    println(io, "atom_style atomic")
    println(io, "boundary p p p")
    println(io, "")
    println(io, "read_data $data_file")
    println(io, "")
    println(io, "plugin load $plugin_path")
    println(io, "pair_style ace/minimal")
    println(io, "pair_coeff * * $model_dir Si")
    println(io, "")
    println(io, "neighbor 2.0 bin")
    println(io, "neigh_modify delay 0 every 1 check yes")
    println(io, "")
    println(io, "thermo_style custom step pe")
    println(io, "thermo 1")
    println(io, "")
    println(io, "run 0")
end

# Run LAMMPS
println("\n[7] Running LAMMPS...")
log_file = joinpath(@__DIR__, "build", "log_direct.lammps")
lammps_cmd = `$lammps_exe -in $input_file -log $log_file`

println("   Command: $lammps_cmd")
println("   Plugin: $plugin_path")
println("   Model: $model_dir")
println("   C Interface: $c_interface_path")

try
    run(lammps_cmd)
    println("\n✓ LAMMPS ran successfully!")

    # Show the last few lines of output
    log_content = read(log_file, String)
    lines = split(log_content, '\n')
    println("\nLAMMPS output (last 20 lines):")
    println("="^80)
    for line in lines[max(1, end-20):end]
        println(line)
    end
    println("="^80)

catch e
    @error "LAMMPS execution failed" exception=e

    # Show log file if it exists
    if isfile(log_file)
        println("\nLAMMPS log file:")
        println("="^80)
        println(read(log_file, String))
        println("="^80)
    end
    exit(1)
end

println("\n" * "="^80)
println("✓ LAMMPS ACE/Minimal Direct Test Complete")
println("="^80)
