...
function build_deployment(
    ...
    # Step 5: Copy Python wrapper (if requested)
    if include_python
        verbose && println("\n[5/6] Including Python/ASE wrapper...")
        python_dir = joinpath(deploy_dir, "python")
        mkpath(python_dir)

        # Copy calculator
        calc_src = joinpath(export_dir, "python", "ace_calculator.py")
        if isfile(calc_src)
            cp(calc_src, joinpath(python_dir, "ace_calculator.py"); force=true)
            verbose && println("  → Copied: ace_calculator.py")
        end
...
function create_python_example!(python_dir::String, name::String, verbose::Bool)
    ...
    write(example_path, """
        ...
        calc = ACECalculator("../lib/libace_$(name).so")
...
function create_readme!(deploy_dir::String, name::String, include_lammps::Bool, include_python::Bool, verbose::Bool)
    ...
    if include_python
        ...
        2. Use in Python:
           ```python
           from python.ace_calculator import ACECalculator
           from ase.build import bulk
