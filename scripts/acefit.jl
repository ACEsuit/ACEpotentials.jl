...
params = load_dict(args["params"])
#fit_params = fill_defaults(raw_params)  # TODO
...
model = acemodel(elements = [Symbol(e) for e in params["elements"]],
                 order = params["order"],
                 totaldegree = params["totaldegree"],
                 rcut = params["rcut"])
    

solver = ACEfit.BayesianLinearRegressionSVD()  # TODO
data = read_extxyz(params["datafile"])
acefit!(model,
        data;
        solver = solver,
